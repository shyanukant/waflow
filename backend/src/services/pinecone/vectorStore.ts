...
export const searchKnowledge = async (
    ...
    try {
        ...
        // Build filter if knowledgeBaseIds provided (filter out null/undefined values)
        let filter: Record<string, any> | undefined;
        if (knowledgeBaseIds && knowledgeBaseIds.length > 0) {
            // Filter out any null, undefined, or empty string values
            const validIds = knowledgeBaseIds.filter(id => id != null && id !== '');
            if (validIds.length > 0) {
                filter = { documentId: { $in: validIds } };
                console.log(`Filtering by knowledge IDs: ${validIds.length} valid items`);
            } else {
                console.log(`⚠️ No valid knowledge IDs to filter by (all were null/empty)`);
            }
        }
